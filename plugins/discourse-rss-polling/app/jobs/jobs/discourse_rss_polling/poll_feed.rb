  ...
  module DiscourseRssPolling
    class PollFeed < ::Jobs::Base
      def execute(args)
        ...
        return if !args[:force] && feed_disabled?
      ...
      def resolve_author(args)
        redacted = ::DiscourseRssPolling::FeedUrl.redact(args[:feed_url])
        ...
        if args[:user_id]
          ...
          Rails.logger.warn(
            "RSS Polling: user_id #{args[:user_id]} not found for feed #{redacted}, falling back to system user",
          )
          ...
          Rails.logger.warn(
            "RSS Polling: username '#{args[:author_username]}' not found for feed #{redacted}, falling back to system user",
          )
        end
