...
module DiscoursePostEvent
  class EmailRenderer
    ...
    def dates
      ...
      formatted = "#{format_date(starts_at)}#{timezone_suffix}"

      ends_at = event_node["data-end"]
      formatted = "#{formatted} → #{format_date(ends_at)}#{timezone_suffix}" if ends_at
    ...
    def format_date(value)
      ...
      all_day? ? parsed.strftime("%B %-d, %Y") : parsed.strftime("%B %-d, %Y %-I:%M %p")
    ...
    def status_label
      ...
      key = @event.standalone? ? "standalone" : @event.private? ? "private" : "public"
    ...
    def creator_label
      ...
      username = user.name.presence || user.username
      "#{card_t("created_by")} #{CGI.escape_html(username)}"
    end
    ...
    def starts_at
      event_node["data-start"]
    end

    def timezone
      event_node["data-timezone"] || "UTC"
    end

    def all_day?
      event_node["data-all-day"] == "true"
    end
  end
end
